    ...
    M: Quantizable,
{
    ...
    /// Set the quantized variant directly from a pre-built quantized module.
    /// Use this when loading pre-quantized weights without re-quantizing.
    pub fn set_quantized(&mut self, quantized: M::Quantized) {
        *self = MaybeQuantized::Quantized(quantized);
    }
