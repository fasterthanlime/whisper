...
impl ParakeetDecoder {
    fn frame_token_confidence(logits_t: ndarray::ArrayView1<'_, f32>, token_idx: usize) -> f32 {
        let max_logit = logits_t.iter().copied().fold(f32::NEG_INFINITY, f32::max);
        ...
        ((logits_t[token_idx] - max_logit).exp() / denom).clamp(0.0, 1.0)
    }
    ...
    fn ctc_collapse_with_frames(
        &self,
        token_ids: &[(u32, usize, f32)],
    ) -> Vec<(u32, usize, usize, f32, usize)> {
        let mut result: Vec<(u32, usize, usize, f32, usize)> = Vec::new();
        let mut prev_token: Option<u32> = None;

        for &(token_id, frame, confidence) in token_ids.iter() {
            if token_id == self.pad_token_id as u32 {
                prev_token = Some(token_id);
                continue;
            }
            ...
            if Some(token_id) != prev_token {
                ...
                // Start new token
                result.push((token_id, frame, frame, confidence, 1));
    ...
    pub fn decode_with_timestamps(
        ...
        for t in 0..time_steps {
            ...
            let confidence = Self::frame_token_confidence(logits_t, max_idx);
            token_ids_with_frames.push((max_idx as u32, t, confidence));
        }
        ...
        let token_ids: Vec<u32> = collapsed_with_frames
            .iter()
            .map(|(id, _, _, _, _)| *id)
            .collect();
        ...
        for (i, (_token_id, start_frame, end_frame, confidence_sum, confidence_count)) in
            collapsed_with_frames.iter().enumerate()
        {
            ...
            if let Ok(curr_decode) = self.tokenizer.decode(&token_ids_so_far, true) {
                ...
                if !added_text.is_empty() {
                    ...
                    timed_tokens.push(TimedToken {
                        ...
                        confidence: Some(*confidence_sum / (*confidence_count as f32)),
                    });
                }
