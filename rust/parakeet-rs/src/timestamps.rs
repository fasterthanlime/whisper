...
fn combine_confidence(tokens: &[TimedToken]) -> Option<f32> {
    let mut weighted = 0.0_f32;
    let mut total_weight = 0.0_f32;
    let mut sum = 0.0_f32;
    let mut count = 0_u32;
    for token in tokens {
        let Some(conf) = token.confidence else {
            continue;
        };
        let weight = (token.end - token.start).max(0.0);
        if weight > 0.0 {
...
pub(crate) fn group_by_words(tokens: &[TimedToken]) -> Vec<TimedToken> {
    ...
    let mut current_word_token_start = 0_usize;
    ...
    if !current_word_text.is_empty() {
        ...
        if word_lower != last_word_lower {
            words.push(TimedToken {
                ...
                confidence: combine_confidence(&tokens[current_word_token_start..]),
            });
        }
    }
        ...
        if ends_sentence {
            ...
            if !sentence_text.is_empty() {
                sentences.push(TimedToken {
                    ...
                    confidence: combine_confidence(&current_sentence),
                });
            }
...
mod tests {
    ...
    #[test]
    fn test_word_grouping_aggregates_confidence() {
        let tokens = vec![
            TimedToken {
                text: "▁bear".to_string(),
                start: 0.0,
        ...
        let conf = words[0].confidence.expect("word confidence");
        assert!((conf - 0.6666667).abs() < 1e-4, "got {conf}");
    }
}
