...
impl QuantizedEmbedding {
    ...
    /// Create a quantized embedding directly from pre-quantized weight data.
    pub fn from_parts(
        weight: Array,
        scales: Array,
        biases: Array,
...
impl QuantizedLinear {
    ...
    /// Create a quantized linear layer directly from pre-quantized weight data.
    ///
    /// Use this when loading pre-quantized weights from safetensors (e.g. MLX
    /// community models) where weight/scales/biases are already in the correct
    /// format. No quantization computation is performed.
    pub fn from_parts(
        weight: Array,
        bias: Option<Array>,
        scales: Array,
