...
impl ForcedAligner {
    ...
    pub fn align(&mut self, samples: &[f32], text: &str) -> Result<Vec<ForcedAlignItem>, Exception> {
        ...
        let before_ids: Vec<i32> = input_ids[..audio_start_pos].iter().map(|&x| x as i32).collect();
        let after_ids: Vec<i32> = input_ids[audio_start_pos + n_audio_tokens..].iter().map(|&x| x as i32).collect();
        ...
        let output_vec: Vec<i32> = (0..seq_len).map(|i| {
            output_ids.index(i as i32).item::<i32>()
        }).collect();
    ...
    fn build_prompt(&self, n_audio_tokens: usize, words: &[String]) -> Result<(Vec<i64>, usize), Exception> {
        ...
        for word in words {
            let enc = self.tokenizer.encode(word.as_str(), false)
        ...
        if !is_normal[i] {
            ...
            if anomaly_count <= 2 {
                ...
                match (left_val, right_val) {
                    ...
                    (Some(l), None) => {
                        for k in i..j { result[k] = l; }
                    }
                    (None, Some(r)) => {
                        for k in i..j { result[k] = r; }
                    }
                    (None, None) => {}
                }
            }
