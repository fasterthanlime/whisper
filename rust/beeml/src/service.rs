...
use bee_phonetic::{
    ...
    TranscriptAlignmentToken, align_token_sequences_with_left_word_boundaries,
    enumerate_transcript_spans_with, feature_similarity, normalize_ipa_for_comparison,
    normalize_ipa_for_comparison_with_spans, phoneme_similarity, query_index, reduce_ipa_tokens,
    score_shortlist, sentence_word_tokens,
};
use bee_transcribe::g2p::CachedEspeakG2p;
use bee_transcribe::zipa_align::{self, timed_range_for_normalized_range};
...
impl BeeMlService {
    ...
    pub(crate) fn build_transcribe_phonetic_trace(
        ...
        let utterance_zipa_normalized = utterance_zipa_normalized_with_spans
            ...
            .collect::<Vec<_>>();
        let transcript_word_raw_ranges =
            zipa_align::transcript_word_raw_ranges(&mut g2p, transcript)?;
        ...
        let transcript_token_ranges = (0..transcript_word_normalized_ranges.len())
            .map(|word_index| {
                zipa_align::transcript_token_range_for_span(
        ...
        let word_alignments = transcript_word_normalized_ranges
            ...
            .filter_map(|(word_index, (_, transcript_normalized))| {
                ...
                let zipa_raw = zipa_align::raw_slice_for_normalized_range(
                    &utterance_zipa_raw,
                    &utterance_zipa_normalized_with_spans,
                    zipa_norm_range.clone(),
                );
        ...
        for span in spans {
            let transcript_normalized = zipa_align::transcript_normalized_for_span(
                &transcript_word_normalized_ranges,
                span.token_start,
                span.token_end,
            );
            ...
            let transcript_norm_range = zipa_align::transcript_token_range_for_span(
                &transcript_word_normalized_ranges,
                span.token_start,
                span.token_end,
            );
            let Some(selection) = zipa_align::select_span_alignment_range(
