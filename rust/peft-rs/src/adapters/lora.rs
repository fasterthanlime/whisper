...
impl SaveLoad for LoraLayer {
    ...
    fn load_state_dict(&mut self, state_dict: HashMap<String, Tensor>) -> Result<()> {
        let Some(lora_a_weight) = state_dict.get("lora_a.weight") else {
            return Err(PeftError::WeightLoad(
                "Missing required keys in state_dict".to_string(),
            ));
        };
        let Some(lora_b_weight) = state_dict.get("lora_b.weight") else {
            return Err(PeftError::WeightLoad(
                "Missing required keys in state_dict".to_string(),
            ));
        };
        ...
        self.lora_a = Linear::new(lora_a_weight.clone(), None);
        self.lora_b = Linear::new(lora_b_weight.clone(), None);
...
mod tests {
    ...
    fn test_lora_save_load_weights() -> Result<()> {
        ...
        let lora_b_diff = loaded_state["lora_b.weight"]
            ...
            .max(0)?
            .to_scalar::<f32>()?;
        assert!(lora_a_diff < 1e-6);
        assert!(lora_b_diff < 1e-6);
