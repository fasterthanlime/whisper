...
function color(label) {
  if (label === "trace-ui") return "\x1b[38;5;81m";
...
function run(label, command, args) {
  ...
  child.on("exit", (code, signal) => {
    const status = signal != null ? `signal ${signal}` : `exit code ${code ?? 0}`;
  ...
  child.on("error", (error) => {
    process.stderr.write(`${color(label)}[${label}:error]\x1b[0m ${error.message}\n`);
    shutdown();
    process.exitCode = 1;
  });
}
...
run("trace-api", "node", ["debug/cut-trace-server.js"]);
run("trace-ui", "pnpm", ["--dir", "cut-trace-web", "dev"]);
