...
impl Module for RmsNorm {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        if xs.is_contiguous() {
            // Some backends only implement rms_norm kernels for matching
            // dtype pairs (e.g. BF16/BF16, F32/F32). Cast the norm weights to
            // the activation dtype when needed so mixed BF16/F32 inference can
            // still use the fast contiguous rms_norm op.
            if self.0.weight.dtype() == xs.dtype() {
                crate::ops::rms_norm(xs, &self.0.weight, self.0.eps as f32)
            } else {
                let alpha = self.0.weight.to_dtype(xs.dtype())?;
                crate::ops::rms_norm(xs, &alpha, self.0.eps as f32)
            }
        } else {
            self.0.forward(xs)
        }
