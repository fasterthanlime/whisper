...
impl ParakeetTDTModel {
    ...
    fn token_confidence(logits: &[f32], token_id: usize) -> f32 {
        ...
        ((logits[token_id] - max_logit).exp() / denom).clamp(0.0, 1.0)
    }

    /// Run greedy decoding - returns (token_ids, frame_indices, durations, confidences)
    pub fn forward(
        &mut self,
        features: Array2<f32>,
        ...
        let (tokens, frame_indices, durations, confidences) =
            self.greedy_decode(&encoder_out, encoder_len)?;

        Ok((tokens, frame_indices, durations, confidences))
    }
    ...
    fn greedy_decode(
        ...
        while t < time_steps {
            ...
            if token_id != blank_id {
                ...
                confidences.push(Self::token_confidence(&vocab_logits, token_id));
