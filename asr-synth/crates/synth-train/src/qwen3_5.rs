//! Quantized Qwen3.5 model implementation, vendored from candle PR #3396
//! and adapted to the local Candle 0.9.2 API with MLX LoRA overlays.

use candle_core::{
    D, DType, Device, Module, Result, Tensor,
    quantized::{QMatMul, QTensor, gguf_file},
};
use candle_nn::{Activation, Embedding, kv_cache::ConcatKvCache};
...
fn full_attention_module_name(layer_idx: usize, projection: &str) -> String {
    format!("model.layers.{layer_idx}.self_attn.{projection}")
}

fn linear_attention_module_name(layer_idx: usize, projection: &str) -> String {
    format!("model.layers.{layer_idx}.linear_attn.{projection}")
}
...
fn apply_lora(
    ...
    match lora_layers.get(module_name.as_ref()) {
        Some(layer) => layer
...
impl<R: Read + Seek> Gguf<R> {
    ...
    fn qmatmul(&mut self, name: &str) -> Result<(QMatMul, usize)> {
...
impl AttentionWeights {
    ...
    fn forward(
        ...
        let q_out = apply_lora(
            lora_layers,
            full_attention_module_name(layer_idx, "q_proj"),
            x,
        ...
        let k = apply_lora(
            lora_layers,
            full_attention_module_name(layer_idx, "k_proj"),
            x,
            self.k_proj.forward(x)?,
        )?;
        let v = apply_lora(
            lora_layers,
            full_attention_module_name(layer_idx, "v_proj"),
            x,
        ...
        apply_lora(
            lora_layers,
            full_attention_module_name(layer_idx, "o_proj"),
            &reshaped_ctx,
...
impl GatedDeltaNetWeights {
    ...
    fn new<R: Read + Seek>(
        ...
        let conv1d_weight = gg
            ...
            .unsqueeze(1)?;
        let conv1d_weight_decode = conv1d_weight.squeeze(1)?.unsqueeze(0)?;
    ...
    fn decode_conv1d_step(&self, window: &Tensor) -> Result<Tensor> {
        let weighted = window.broadcast_mul(&self.conv1d_weight_decode)?;
        weighted.sum_keepdim(D::Minus1)
    }
    ...
    fn forward(
        ...
        let mixed_qkv = apply_lora(
            lora_layers,
            linear_attention_module_name(layer_idx, "in_proj_qkv"),
            hidden_states,
        ...
        let z = apply_lora(
            lora_layers,
            linear_attention_module_name(layer_idx, "in_proj_z"),
            hidden_states,
        ...
        let b = apply_lora(
            lora_layers,
            linear_attention_module_name(layer_idx, "in_proj_b"),
            hidden_states,
            self.in_proj_b.forward(hidden_states)?,
        )?;
        let a = apply_lora(
            lora_layers,
            linear_attention_module_name(layer_idx, "in_proj_a"),
            hidden_states,
        ...
        let mixed_qkv = if use_precomputed_states {
            ...
            *conv_state = conv_state_data.narrow(2, 1, self.conv_kernel_size - 1)?;
            let out = self.decode_conv1d_step(&conv_state_data)?;
        ...
        apply_lora(
            lora_layers,
            linear_attention_module_name(layer_idx, "out_proj"),
            &core_attn_out,
    ...
    fn forward(
        ...
        let x = match &mut self.token_mixer {
            TokenMixer::FullAttention(attn) => {
                attn.forward(&x, mask, offset, layer_idx, lora_layers)?
            }
            TokenMixer::LinearAttention(attn) => attn.forward(&x, layer_idx, lora_layers)?,
        };
        let x = (residual + x)?;

        let residual = &x;
        let x = self.post_attention_layernorm.forward(&x)?;
        let x = self.mlp.forward(&x, layer_idx, lora_layers)?;
        x + residual
    }
...
impl ModelWeights {
    ...
    pub fn forward(&mut self, input: &Tensor, offset: usize) -> Result<Tensor> {
        ...
        for (layer_idx, layer) in self.layers.iter_mut().enumerate() {
            h = layer.forward(&h, causal_mask.as_ref(), offset, layer_idx, lora_layers)?;
        }
