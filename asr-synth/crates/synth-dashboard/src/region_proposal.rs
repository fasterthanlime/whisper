...
pub fn enumerate_transcript_spans_with<F>(
    transcript: &str,
    max_span_words: usize,
    alignments: Option<&[qwen3_asr::ForcedAlignItem]>,
    mut ipa_for_text: F,
) -> Vec<TranscriptSpan>
where
...
mod tests {
    ...
    fn enumerate_transcript_spans_skips_empty_ipa() {
        let spans = enumerate_transcript_spans_with("serde_json", 2, None, |_text| None);
